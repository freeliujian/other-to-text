...
const VideoToText = (props:VideoToTextProps) => {
  ...
  const captureImage = function () {  
      ...
      if (cavRef.current.width) {  
          ...
          ctx.drawImage(videoRef.current, 0, 0, cavRef.current.width, cavRef.current.height);  
          setCode( toCharts({context:ctx, width:cavRef.current.width, height:cavRef.current.height,rowChars: 100}));
  ...
  return (
    <div>
      ...
      <br/>
      <video ref={videoRef} style={{width:'200px',height:'200px'}} src={'http://localhost:8002//static/badapple.7ce6c6ad.mp4'} controls/>
